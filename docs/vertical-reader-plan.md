...
| 正文段落 | 连续填列;**换段即换列**,新段顶格(古籍无空两格惯例) |
| 偈颂(检测命中) | 按句折列:句长 n,每列装 `floor(charsPerCol/n)` 个整句,列底余格留空;句间不空格。所有偈颂列同构 → 句首横向自动对齐成行 |
...
**偈颂检测启发式**(`verse_detector.dart`):对每个段落,按 `。,;!?` 切句 → 句数 ≥ 4 且全部句长相等且 ∈ {4,5,6,7} → 判定为 n 言偈颂。混合段落不检测(canon 中偈颂几乎总是独立段落);误判风险偏向漏检(漏检=按散文连排,视觉仍对齐,无害)。
